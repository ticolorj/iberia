...
def search_leg_offers(token, origin, destination, date_iso):
    """Búsqueda por tramo, Iberia, ECONOMY cabin, branded fares ON"""
...
def offer_is_for_exact_time(offer, origin, destination, date_iso, time_hhmm):
    """Compara la hora exacta local de salida del vuelo"""
    ...
    at = dep.get("at", "")
    if dep.get("iataCode") != origin or arr.get("iataCode") != destination:
        return False
...
def best_economy_price(data, origin, destination, date_iso, time_hhmm):
    """Devuelve la mejor oferta Economy para esa hora exacta"""
...
def main():
    ...
    lines = [f"[Iberia Watch — Economy Fare] {now}", "Precios actuales por tramo exacto:", ""]
    ...
    for (o, d, date_iso, hhmm, label) in LEGS:
        ...
        try:
            ...
            if not best:
                lines.append(f"• {label}: (Economy no disponible)")
                continue
            ...
            lines.append(f"• {label}: {CURRENCY} {price:.2f}{delta}   Salida: {dep}   [Economy]")
